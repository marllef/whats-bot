...
func Instance(index string) *whatsmeow.Client {
	flag.Parse()
	if *debugLogs {
		logLevel = "DEBUG"
	}
	...
	jid, err := types.ParseJID(index)
	log.Errorf("JID %v", jid)
	...
	device, err := storeContainer.GetDevice(jid)
	if err != nil {
		log.Errorf("Failed to get device: %v", err)
		return nil
	}
	log.Errorf("Device %v", *device)
...
func handleCmd(cmd string, args []string) {
	...
	case "revoke":
		...
		fmt.Println(cli.Store.Account.String())
	case "sendimg":
		if len(args) < 2 {
...
func handler(rawEvt interface{}) {
	...
	case *events.Message:
		if evt.Info.Chat.Server == "broadcast" {
			fmt.Println("Skipping Broadcast")
			return
		}

		if evt.Info.Chat.Server == "g.us" {
			fmt.Println("Skipping Group")
			return
		}
		...
		args := []string{evt.Info.Chat.User, fmt.Sprintf("Eu Sou, %s", cli.Store.PushName)}
		...
		fmt.Println("Device: ")
		fmt.Println(evt.Info.Chat.Server)
		fmt.Println("User: ")
		fmt.Println(evt.Info.Chat.User)

		img := evt.Message.GetImageMessage()
		if img != nil {
			fmt.Println("Skipping Images")
			return
	...
	case *events.HistorySync:
		fmt.Print("Skipping File creation")
		return
